...
def main():  # noqa: C901
    ...
    parser.add_argument(
        "--env", help="environment ID", type=str, default="CartPole-v1"
    )
    parser.add_argument(
        "-f",
        "--folder",
    ...
    if args.episodes > 0:
        args.n_timesteps = np.iinfo(np.int32).max

    try:
        for _ in range(args.n_timesteps):
            if args.episodes > 0 and args.episodes <= len(episode_rewards):
                break
            ...
            if done:
                for key, value in events.items():
                    events[key] += infos[0]["events"][key]
                successes.append(infos[0]["events"]["succeed"])
            ...
            if args.n_envs == 1:
                ...
                if done and not is_atari and args.verbose > 0:
                    ...
                    print(
                        f"Episode {len(episode_rewards) + 1} Reward: {episode_reward:.2f} Length: {ep_len}"
                    )
    ...
    if args.verbose > 0 and len(successes) > 0:
        ...
        for key, value in events.items():
            if key == "discomfort":
                print(f"{key}: {value/len(episode_rewards):.2f}")
            else:
                print(f"{key}: {100*value/len(episode_rewards):.2f}%")
